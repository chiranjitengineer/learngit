...
--            
        ...
        IF NVL(LV_LOANCODE,'NA') = 'PFNRL_50' OR NVL(LV_LOANCODE,'NA') = 'PFNRL_85' THEN
            IF NVL(LV_CONTRIBUTION,0) <> NVL(LV_MASTER.AMOUNT,0) THEN
                LV_ERROR_REMARK := 'Validation Failure : [Sanction Amount Should Be Equal To Total Component Value!]' || LV_PF_C ||LV_PF_E||LV_VPF;
                RAISE_APPLICATION_ERROR(TO_NUMBER(FN_DISPLAY_ERROR( 'COMMON')),FN_DISPLAY_ERROR( 'COMMON',5,LV_ERROR_REMARK));
            END IF;
        END IF;
...
--        SELECT * FROM LOANSANCTION_DETAILS        
       ...
       END IF;
        ...
        IF LV_MASTER.OPERATIONMODE ='D' THEN
            ...
            IF LV_CNT > 0 THEN
                LV_ERROR_REMARK := 'Validation Failure : [This Loan can not be deleted.!]';
                RAISE_APPLICATION_ERROR(TO_NUMBER(FN_DISPLAY_ERROR( 'COMMON')),FN_DISPLAY_ERROR( 'COMMON',5,LV_ERROR_REMARK));
...
--            AND TRANSACTIONTYPE='REPAY'
            ...
            AND WORKERSERIAL=lv_master.WORKERSERIAL
            AND TOKENNO=lv_master.TOKENNO
            AND LOANCODE=lv_master.LOANDATE
            AND LOANDATE=lv_master.LOANDATE;
