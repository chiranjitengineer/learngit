CREATE OR REPLACE PROCEDURE EINVOICE_INFRA.PROC_RPT_DEBITCREDITNOTE_IRN
(
    P_COMPANYCODE    varchar2, 
    P_DIVISIONCODE varchar2, 
    P_FROMDATE varchar2,
    P_TODATE varchar2,
    P_DBCRNOTENO varchar2, 
    P_OPTION varchar2  default 'NORMAL', 
    P_CHANNELCODE varchar2 default null,
    P_LOGOPATH1 varchar2,
...
LV_SQLSTR := LV_SQLSTR || '	ITEMMASKALLOW, SPECIALREMARKS, CINNO, REGDADDRESS, RUKA_VS_MR_QTL_FOR_CANC, GSTNO, COMPANYPINCODE, CHARGEDATA,EX1,EX2,EX3' || CHR(10);
LV_SQLSTR := LV_SQLSTR || ')' || CHR(10);
...
LV_SQLSTR := LV_SQLSTR || '--MP' || CHR(10);
LV_SQLSTR := LV_SQLSTR || 'CHARGEDATA, CASE WHEN  SEV.DOCUMENTTYPE LIKE ''%CRNOTE%'' THEN ''CREDIT NOTE'' ELSE ''DEBIT NOTE'' END EX1,' || CHR(10);
LV_SQLSTR := LV_SQLSTR || 'CASE WHEN  SEV.DOCUMENTTYPE LIKE ''%CRNOTE%'' THEN ''Credit Note No'' ELSE ''Debit Note No'' END EX2, fn_num_to_words (SEV.GROSSAMOUNT,''RS'') EX3' || CHR(10);
...
IF P_DBCRNOTENO IS NOT NULL THEN
    LV_SQLSTR := LV_SQLSTR || 'AND DBCRNOTENO IN( '|| P_DBCRNOTENO ||')' || CHR(10);
