...
begin
    ...
    PROC_WPSVIEWCREATION ( P_COMPCODE,P_DIVCODE,'ALL',0,P_FNSTDT,P_FNENDT,P_TABLENAME,P_PROCESSTYPE);   -- ADD BY AMALESH ON 16.02.2021
...
--            ||'             NVL(B.DARATE,0), NVL(B.ADHOCRATE,0),  NVL(B.SPL_ALLOW_RATE,0), NVL(C.VBAMOUNT,0) '||CHR(10);
    ...
    lv_SqlStr := ' INSERT INTO '||P_TABLENAME||' ( '||CHR(10)
            ||'    COMPANYCODE, DIVISIONCODE, SYSROWID, YEARCODE, FORTNIGHTSTARTDATE, FORTNIGHTENDDATE, WORKERSERIAL, TOKENNO, WORKERCATEGORYCODE, DEPARTMENTCODE, SHIFTCODE,'||CHR(10)
            ||'    DARATE, ADHOCRATE, SPL_ALLOW_RATE, '||CHR(10) 
            ...
            ||'    ) '||CHR(10)
            ||'    SELECT A.COMPANYCODE, A.DIVISIONCODE, A.WORKERSERIAL,'''||P_YEARCODE||''',  '''||lv_fn_stdt||''','''||lv_fn_endt||''', A.WORKERSERIAL, A.TOKENNO, A.WORKERCATEGORYCODE, '||chr(10);
            IF P_PROCESSTYPE = 'DAILY WAGES PROCESS' THEN
                lv_SqlStr := lv_SqlStr ||'       A.DEPARTMENTCODE, A.SHIFTCODE, '||CHR(10);
            else
                lv_SqlStr := lv_SqlStr ||'       MAX(A.DEPARTMENTCODE) DEPARTMENTCODE, '' SHIFTCODE '||CHR(10);
            end if;
            lv_SqlStr := lv_SqlStr ||'       NVL(B.DARATE,0) DARATE, NVL(B.ADHOCRATE,0) ADHOCRATE, NVL(B.SPL_ALLOW_RATE,0) SPL_ALLOW_RATE, '||CHR(10)   
            ||'       SUM(ATTENDANCEHOURS) ATTENDANCEHOURS, SUM(NVL(OVERTIMEHOURS,0)) OVERTIMEHOURS,  '||CHR(10)
            ...
            IF P_PROCESSTYPE = 'DAILY WAGES PROCESS' THEN
                lv_SqlStr := lv_SqlStr || '        Z.SHIFTCODE,'||CHR(10);
            END IF;
            
            
            lv_SqlStr := lv_SqlStr || '        SUM(ATTENDANCEHOURS) ATTENDANCEHOURS, SUM(OVERTIMEHOURS) OVERTIMEHOURS,  '||CHR(10)
            ...
            IF P_PROCESSTYPE = 'DAILY WAGES PROCESS' THEN
                lv_SqlStr := lv_SqlStr || '               A.SHIFTCODE,'||CHR(10);
            END IF;
            
            
            lv_SqlStr := lv_SqlStr  ||'               NVL(ATTENDANCEHOURS,0) ATTENDANCEHOURS, NVL(OVERTIMEHOURS,0) OVERTIMEHOURS,   '||CHR(10)  
           ...
           lv_SqlStr :=  lv_SqlStr || '              ) Z '||CHR(10)
            ...
            IF NVL(P_PROCESSTYPE,'NA') = 'DAILY WAGES PROCESS' THEN
                lv_SqlStr := lv_SqlStr || '                 ,DEPARTMENTCODE, SHIFTCODE';
            END IF;
            
            lv_SqlStr :=  lv_SqlStr ||'    ) A, WPSWORKERMAST B, '||CHR(10) 
            ||'    (    '||CHR(10)
...
--            ||'      AND A.DATEOFATTENDANCE >=  '''||lv_fn_stdt||''' AND A.DATEOFATTENDANCE <=  '''||lv_fn_endt||'''   '||CHR(10)
            ...
            ||'      AND A.WORKERSERIAL = C.WORKERSERIAL (+) /* AND A.DEPARTMENTCODE = C.DEPARTMENTCODE (+) */ '||CHR(10);
              if P_WORKKERSERIAL is not null then
                lv_SqlStr := lv_SqlStr   ||'      AND A.WORKERSERIAL IN ('||P_WORKKERSERIAL||')  '||CHR(10);
            END IF;      
            
            --ADDED ON 10/02/2021  
            if NVL(P_PROCESSTYPE,'NA') = 'DAILY WAGES PROCESS' then
                lv_SqlStr := lv_SqlStr   ||'      AND NVL(A.OVERTIMEHOURS,0) > 0  '||CHR(10);
            END IF;       
            --ADDED ON 10/02/2021 
            
--            || '   AND A.WORKERSERIAL = D.WORKERSERIAL (+) /*AND A.SPELLTYPE=D.SPELLTYPE(+) AND A.DATEOFATTENDANCE = D.DATEOFATTENDANCE (+) */'||CHR(10)
            lv_SqlStr := lv_SqlStr ||'    GROUP BY A.COMPANYCODE, A.DIVISIONCODE, A.WORKERSERIAL, A.TOKENNO, A.WORKERCATEGORYCODE, '||CHR(10);
            if NVL(P_PROCESSTYPE,'NA') = 'DAILY WAGES PROCESS' then
                lv_SqlStr := lv_SqlStr ||'    A.DEPARTMENTCODE, A.SHIFTCODE, '||CHR(10);
            END IF;
            lv_SqlStr := lv_SqlStr ||'             NVL(B.DARATE,0), NVL(B.ADHOCRATE,0),  NVL(B.SPL_ALLOW_RATE,0), NVL(C.VBAMOUNT,0),NVL(ADDLBASIC,0), NVL(C.HRS_RATE,0)'||CHR(10)
