...
--           ||'  NVL(CONTRIBUTIONAMOUNT,0)+NVL(PREVIOUSDUEAMOUNT,0) + nvl(C.ELEC_BAL_AMT,0) ELEC_BAL_AMT, EMIAMOUNT '||CHR(10)
           ...
           ||'              SELECT WORKERSERIAL, SUM(NVL(CONTRIBUTIONAMOUNT,0)) ELEC_BAL_AMT  '||CHR(10)
           ||'              FROM ELECTRICMETERREADING   '||CHR(10)
           ||'               WHERE COMPANYCODE= '''||P_COMPCODE||'''    '||CHR(10)
