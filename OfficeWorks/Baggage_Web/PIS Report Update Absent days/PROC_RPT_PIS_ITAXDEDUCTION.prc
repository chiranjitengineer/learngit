CREATE OR REPLACE PROCEDURE BAGGAGE_WEB.PROC_RPT_PIS_ITAXDEDUCTION
(
...
BEGIN
    ...
    LV_RPT_CAPTION:= 'INCOME TAX DEDUCTION FOR THE MONTH OF '||LV_MONTH||', '|| LV_YEAR;
    ...
    AS_XLSX.CELL( 2, 4,'CATEGORY', P_ALIGNMENT => AS_XLSX.GET_ALIGNMENT( P_HORIZONTAL => 'center' ), P_BORDERID => AS_XLSX.GET_BORDER( 'double', 'double', 'thin', 'thin' ),P_FONTID => AS_XLSX.get_font('TAHOMA', p_fontsize => 10, p_BOLD => TRUE) );
    AS_XLSX.CELL( 3, 4,'TOKENNO', P_ALIGNMENT => AS_XLSX.GET_ALIGNMENT( P_HORIZONTAL => 'center' ), P_BORDERID => AS_XLSX.GET_BORDER( 'double', 'double', 'thin', 'thin' ),P_FONTID => AS_XLSX.get_font('TAHOMA', p_fontsize => 10, p_BOLD => TRUE) );
    ...
    AS_XLSX.CELL( 5, 4,'I.TAX', P_ALIGNMENT => AS_XLSX.GET_ALIGNMENT( P_HORIZONTAL => 'center' ), P_BORDERID => AS_XLSX.GET_BORDER( 'double', 'double', 'thin', 'thin' ),P_FONTID => AS_XLSX.get_font('TAHOMA', p_fontsize => 10, p_BOLD => TRUE) );
   
    LV_ROWNUM := 4;

DELETE FROM GTT_ITAXDEDUCTION WHERE 1=1;

LV_SQLSTR := NULL;
LV_SQLSTR := LV_SQLSTR || 'INSERT INTO GTT_ITAXDEDUCTION (SLNO, TOKENNO, WORKERSERIAL, CATEGORYCODE, CATEGORYDESC, EMPLOYEENAME, ITAX)' || CHR(10);
LV_SQLSTR := LV_SQLSTR || 'SELECT ROW_NUMBER() OVER (ORDER BY A.TOKENNO) SLNO,A.TOKENNO, A.WORKERSERIAL, ' || CHR(10);
LV_SQLSTR := LV_SQLSTR || 'C.CATEGORYCODE, C.CATEGORYDESC, A.EMPLOYEENAME, NVL(ITAX,0) ITAX ' || CHR(10);
LV_SQLSTR := LV_SQLSTR || 'FROM PISEMPLOYEEMASTER  A, (SELECT * FROM PISPAYTRANSACTION WHERE YEARMONTH = '''||P_YEARMONTH||''') /*PISPAYTRANSACTION*/ B, PISCATEGORYMASTER C' || CHR(10);
...
EXECUTE IMMEDIATE LV_SQLSTR;
    ...
    FOR C1 IN (  
        SELECT SLNO, TOKENNO, WORKERSERIAL, CATEGORYCODE, CATEGORYDESC, EMPLOYEENAME, ITAX
        FROM GTT_ITAXDEDUCTION     
    )
    LOOP
        ...
        AS_XLSX.CELL( 5, LV_ROWNUM,C1.ITAX, P_BORDERID => AS_XLSX.GET_BORDER( 'thin', 'thin', 'thin', 'thin' ), p_numFmtId => as_xlsx.get_numFmt( '0.00' ) );
       
        LV_TOT_ITAX := LV_TOT_ITAX + C1.ITAX;
   END LOOP;       
    ...
    AS_XLSX.CELL( 5, LV_ROWNUM,LV_TOT_ITAX, P_BORDERID => AS_XLSX.GET_BORDER( 'double', 'double', 'thin', 'thin' ), p_numFmtId => as_xlsx.get_numFmt( '0.00' ), P_FONTID => AS_XLSX.get_font('TAHOMA', p_fontsize => 10, p_BOLD => TRUE) );
