...
begin
    ...
    IF P_ViewName  = 'ATTN' OR NVL(P_ViewName,'ALL') = 'ALL' then
         ...
         IF P_PROCESSTYPE = 'DAILY WAGES PROCESS' THEN  --- ADD BY AMALESH ON 16/02/2021 -----
            lv_SqlStr := lv_SqlStr ||' A.DEPARTMENTCODE, A.SHIFTCODE, MAX(A.OCCUPATIONCODE) AS OCCUPATIONCODE, '||CHR(10);  
         ELSE          
