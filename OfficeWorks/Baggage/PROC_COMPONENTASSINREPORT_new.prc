...
END IF;
    ...
    lv_sqlstr :=  'INSERT INTO GTT_COMPONENTASSINREPORT '||chr(10) 
                ...
                ||'       fn_get_compvalue(C.COMPANYCODE,C.DIVISIONCODE,C.YEARMONTH,C.WORKERSERIAL,B.COMPONENTCODE,'''||p_option||''') COMPVALUE, TO_CHAR(TO_DATE('''||p_yearmonth||''',''YYYYMM''),''MON-YYYY'') PRINTDATE, '||chr(10)
                ...
                if p_option='PIS COMPONENT REGISTER' then
                    lv_sqlstr:=lv_sqlstr||'       ( SELECT * FROM PISPAYTRANSACTION '||chr(10);
                end if;
                ...
                if p_option='PIS COMPONENT REGISTER' then
                    lv_sqlstr:=lv_sqlstr||'                                               FROM PISPAYTRANSACTION WHERE 1=1'||chr(10);
                ...
                ||'                                           ) '||chr(10)
                ||'       ) C, COMPANYMAST M, DIVISIONMASTER V '||chr(10)
