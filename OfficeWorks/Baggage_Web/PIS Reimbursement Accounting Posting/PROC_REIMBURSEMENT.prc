CREATE OR REPLACE PROCEDURE BAGGAGE_WEB.PROC_REIMBURSEMENT
(
...
AS 
    ...
    LV_PAYFORMULA  VARCHAR2(500 BYTE);
...
--   SELECT (TRUNC(TO_DATE('31-DEC-2020')-TO_DATE('01-JAN-2020'))+1) yrdays FROM DUAL
    ...
    BEGIN
        ...
        PRC_PISVIEWCREATION (P_COMPANYCODE,P_DIVISIONCODE,'PISASSIGN',0,P_YEARMONTH,P_YEARMONTH, 'SALARY', 'PISPAYTRANSACTION_SWT');
...
LV_SQLSTR := LV_SQLSTR || '         A.EMPLOYEENAME,TO_CHAR(A.DATEOFJOIN,''DD/MM/YYYY'') DATEOFJOIN , A.UNITCODE,A.CATEGORYCODE, A.GRADECODE, '''||P_COMPONENTCODE||''' COMPONENTCODE, ' || CHR(10);
LV_SQLSTR := LV_SQLSTR || '         B.COMPONENTAMOUNT  COMPONENTAMOUNT,'''||P_COMPONENTCODE||'''   TRANSACTIONTYPE,    ''ADD'' ADDLESS, ' || CHR(10);
...
LV_SQLSTR := LV_SQLSTR || '              (' || CHR(10);
--LV_SQLSTR := LV_SQLSTR || '                  SELECT P.COMPANYCODE, P.DIVISIONCODE,P.UNITCODE, P.GRADECODE, P.CATEGORYCODE, P.DEPARTMENTCODE, P.TOKENNO, P.WORKERSERIAL,  P.YEARMONTH , '||NVL(LV_PAYFORMULA,0)||' BASIC' || CHR(10);
...
LV_SQLSTR := LV_SQLSTR || '                  SELECT P.COMPANYCODE, P.DIVISIONCODE,P.UNITCODE, P.GRADECODE, P.CATEGORYCODE, P.TOKENNO, P.WORKERSERIAL,  P.COMPONENTAMOUNT BASIC    ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  FROM PISREIMBURSEMENT_ENTITLE   P    ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  WHERE 1=1     ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  AND P.TRANSACTIONTYPE=''ENTITLEMENT''     ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  AND P.YEARCODE =  '''||P_YEARCODE||'''    ' || CHR(10); 
...
LV_SQLSTR := LV_SQLSTR || '                  WHERE 1=1     ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  AND P.TRANSACTIONTYPE=''ENTITLEMENT''     ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  AND P.YEARCODE =  '''||(SUBSTR(P_YEARCODE,1,4) -1)||'-'||(SUBSTR(P_YEARCODE,-4)-1)||'''    ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  AND COMPONENTCODE='''||P_COMPONENTCODE||'''    ' || CHR(10); 
LV_SQLSTR := LV_SQLSTR || '                  AND COMPANYCODE='''||P_COMPANYCODE||'''' || CHR(10);
LV_SQLSTR := LV_SQLSTR || '                  AND DIVISIONCODE='''||P_DIVISIONCODE||'''' || CHR(10);
...
IF NVL(P_OPERATIONMODE,'A') = 'A' THEN
...
LV_SQLSTR := LV_SQLSTR || '         AND A.DIVISIONCODE='''||P_DIVISIONCODE||''' ' || CHR(10);
    ...
    IF P_TOKENNO IS NOT NULL THEN
        LV_SQLSTR := LV_SQLSTR || '   AND A.TOKENNO='''||P_TOKENNO||'''' || CHR(10);
    END IF;
