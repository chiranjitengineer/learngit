CREATE OR REPLACE PROCEDURE BIRLANEW.PROC_RPT_UCO_ATMLIST_TEXT
(
...
--AND LENGTH(BANKACCNUMBER) = 15
    ...
    LV_SQLSTR := LV_SQLSTR || 'AND IFSCCODE LIKE ''UCBA0000306'''|| chr(10) ;    
...
----------------------------------------------------------------------------------------------
            ...
            LV_INSERTSTR := ' UCO BANK ATM LIST FOR THE MONTH OF '||TO_CHAR(TO_DATE(P_YEARMONTH,'YYYYMM'),'MON-YY');
            P_INT1 := P_INT1 + 1;
            PROC_INSERT_TEXT_REPORT(LV_INSERTSTR);
