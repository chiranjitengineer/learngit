...
LV_SQLSTR := LV_SQLSTR || 'NVL(PEN_GROSS,0)  PEN_GROSS,round(NVL(PEN_GROSS,0)*0.0833,2) PENSION , NVL(ATTN_WRKD,0) WORKINGDAYS ' || CHR(10);
LV_SQLSTR := LV_SQLSTR || 'FROM PISEMPLOYEEMASTER  A, --PISPAYTRANSACTION B' || CHR(10);
LV_SQLSTR := LV_SQLSTR || '(SELECT * FROM PISPAYTRANSACTION WHERE YEARMONTH = '''||P_YEARMONTH||''') B' || CHR(10);
