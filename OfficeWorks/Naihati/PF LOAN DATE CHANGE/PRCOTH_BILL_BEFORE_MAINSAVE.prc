CREATE OR REPLACE procedure BIRLANEW.prcOth_Bill_Before_MainSave
is
lv_cnt                  number;
...
--   END LOOP;
  ...
  END IF;
   ...
   if lv_ChannelMaster.channeltag  in ('GOVERNMENT') then  
   ...
   SELECT SUBSTR (GSTNNO, 3, 10) 
              INTO lv_DIVISIONPANNO 
               FROM DIVISIONMASTER
                WHERE COMPANYCODE=lv_Master.companycode
                  AND DIVISIONCODE=lv_Master.divisioncode;
