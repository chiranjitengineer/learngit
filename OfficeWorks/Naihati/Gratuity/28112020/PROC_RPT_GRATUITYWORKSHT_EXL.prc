CREATE OR REPLACE PROCEDURE PROC_RPT_GRATUITYWORKSHT_EXL
(
...
--    LV_SQLSTR := 'PROC_RPT_GRATUITYWORKSHT_EXL('''||P_COMPANYCODE||''','''||P_DIVISIONCODE||''','''||P_YEARCODE||''','''||P_YEARMONTH||''')';
    ...
    LV_SQLSTR := LV_SQLSTR || '    PFMEMBERSHIPDATE,DATEOFTERMINATION,TRUNC(MONTHS_BETWEEN(DATEOFTERMINATION,PFMEMBERSHIPDATE)/12)+1  SERVICE_YEAR ' || CHR(10);
    ...
    IF NVL(LV_LAST_TOKENNO,'NA') <> C1.TOKENNO THEN
        ...
        IF LV_LAST_TOKENNO IS NOT NULL THEN
            ...
            LOOP
                ...
                LV_EXCELVALUES :=  'TOTAL^^^^^^^^'||C2.TOTAL_NO;

                LV_EXCELROWSTYLE := 'MERGE~A'||LV_ROWINDEX||'~FONT.SIZE~10~FONT.BOLD~TRUE~RANGE~I'||LV_ROWINDEX||':'||LV_CHR||LV_ROWINDEX||'~FONT.SIZE~10~FONT.BOLD~TRUE~HORIZONTALALIGNMENT~RIGHT~NUMBERFORMAT~##,##,##0.00';
...
--        LV_EXCELROWSTYLE := 'RANGE~A'||LV_ROWINDEX||':I'||LV_ROWINDEX||'~BORDER.BOTTOM.STYLE~THIN~HORIZONTALALIGNMENT~CENTERCONTINUOUS~VERTICALALIGNMENT~CENTER~FONT.SIZE~10';
    ...
    LV_EXCELVALUES := C1.CALENDARYEAR||'^'||C1.STRIKE_DAYS||'^'||C1.ATTENDANCE||'^'||C1.STL
