...
--    PRC_LOANBALANCE_UPDT_WAGES(P_COMPCODE,P_DIVCODE,P_YEARCODE,TO_CHAR(lv_fn_stdt,'DD/MM/YYYY'),TO_CHAR(lv_fn_endt,'DD/MM/YYYY'),'WPS',P_TABLENAME,'PF',NULL,NULL,NULL);
    ...
    PROC_UNREALIZED_BLNC (P_COMPCODE,P_DIVCODE,TO_CHAR(lv_fn_stdt,'DD/MM/YYYY'),TO_CHAR(lv_fn_endt,'DD/MM/YYYY'),'CASH_REALIZE', P_YEARCODE ,'WPS','YES',NULL);    
        ...
        CASE cWages.COMPONENTGROUP
            ...
            WHEN 'UNREALIZED' THEN
                ...
                BEGIN
                    ...
                    SELECT NVL(COMPONENT_EMI,0) INTO lv_ComponentAmt 
                    FROM GBL_UNREALIZED_BLNC
                    WHERE WORKERSERIAL = cWages.WorkerSerial and MODULE = 'WPS'
                    AND COMPONENTCODE = cWages.COMPONENTSHORTNAME; 
            ...
            --ENDED ON 16/03/2021 FOR UNREALIZED CASH OT REALIZED     
            else
              lv_ComponentAmt := cWages.COMPONENTAMOUNT;    
        end case;  
...
--    PROC_PFLOANBLNC(P_COMPCODE,P_DIVCODE,TO_CHAR(lv_fn_stdt,'DD/MM/YYYY'),TO_CHAR(lv_fn_endt,'DD/MM/YYYY'),'','','WPS');
 ...
 -- CASH OT DATA REALIZED/ UNREALIZED 
    lv_remarks := 'CASH OT UNREALIZED DATA INSERT';
    PRC_REALIZEDDATA_INSERT (P_COMPCODE,P_DIVCODE,P_YEARCODE,TO_CHAR(lv_fn_stdt,'DD/MM/YYYY'),TO_CHAR(lv_fn_endt,'DD/MM/YYYY'),'WPS', P_TABLENAME, 'CASH_REALIZE','OTHR_DEDN', 'ALL', NULL);
