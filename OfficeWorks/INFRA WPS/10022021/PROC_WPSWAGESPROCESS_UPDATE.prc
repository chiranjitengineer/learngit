CREATE OR REPLACE PROCEDURE INFRA_WPS.PROC_WPSWAGESPROCESS_UPDATE (P_COMPCODE Varchar2, 
                                                  P_DIVCODE Varchar2, 
                                                  P_YEARCODE Varchar2,
                                                  P_FNSTDT Varchar2, 
                                                  P_FNENDT Varchar2,
...
begin
   ...
   --  DBMS_OUTPUT.PUT_LINE('0_0'); 
    ...
    ---- START PREVIOUS FORTNIGHT DATA TABLE CREATION ON 03.05.2017 -----
    BEGIN 
        EXECUTE IMMEDIATE 'DROP TABLE WPSTEMPPRVFNCOMP CASCADE CONSTRAINTS';
        EXECUTE IMMEDIATE 'CREATE TABLE WPSTEMPPRVFNCOMP AS SELECT * FROM PRVFNCOMP';
      EXCEPTION WHEN OTHERS THEN 
        EXECUTE IMMEDIATE 'CREATE TABLE WPSTEMPPRVFNCOMP AS SELECT * FROM PRVFNCOMP';
    END;
    --DBMS_OUTPUT.PUT_LINE('1_2');
    ---- END PREVIOUS FORTNIGHT DATA TABLE CREATION ON 03.05.2017 -----
    ...
    lv_Component := Replace(lv_Component, 'PRVFNCOMP', 'WPSTEMPPRVFNCOMP');
    --DBMS_OUTPUT.PUT_LINE('2_0');
    
    lv_Sql := 'CREATE TABLE '||P_PHASE_TABLENAME||' AS '||CHR(10)
            || 'SELECT WPSTEMPATTN.WORKERSERIAL,WPSTEMPATTN.DEPARTMENTCODE,WPSTEMPATTN.WORKERCATEGORYCODE, '||CHR(10)  
            ...
            ||' '|| lv_Component ||chr(10) 
            ||' FROM WPSTEMPMAST, WPSTEMPATTN, WPSTEMPCOMPONENT, WPSTEMPPRVFNCOMP '||chr(10)   
            ...
            ||'   AND WPSTEMPATTN.WORKERSERIAL = WPSTEMPPRVFNCOMP.WORKERSERIAL (+) '||CHR(10);               --- NEW ADD ON 03.05.2017 FOR PREVIOUS FNORTHNIGHT
    if P_WORKERSERIAL is not null then
        lv_Sql := lv_Sql ||' AND WPSTEMPMAST.WORKERSERIAL IN ('||P_WORKERSERIAL||')' ||CHR(10); 
    end if; 
    lv_Sql := lv_Sql ||' GROUP BY WPSTEMPATTN.WORKERSERIAL,WPSTEMPATTN.DEPARTMENTCODE,WPSTEMPATTN.WORKERCATEGORYCODE '||chr(10);
    --DBMS_OUTPUT.PUT_LINE('3_0');        
