...
begin
    ...
    IF P_PROCESSTYPE = 'DAILY WAGES PROCESS' THEN  --- NEW ADD BY AMALESH ON 25/02/2021
        lv_Sql := lv_Sql ||'  WPSTEMPATTN.SHIFTCODE,WPSTEMPCOMPONENT.DEPARTMENTCODE,' ||CHR(10); 
    end if;   
           
   lv_Sql := lv_Sql ||' SUM(WPSTEMPATTN.ATTENDANCEHOURS) ATTENDANCEHOURS, SUM(WPSTEMPATTN.STLHOURS) STLHOURS /* STATUTORYHOURS */, SUM(WPSTEMPATTN.OVERTIMEHOURS) OVERTIMEHOURS, ' ||chr(10)
...
--    insert into wps_error_log(PROC_NAME,ORA_ERROR_MESSG,ERROR_QUERY,PAR_VALUES,REMARKS ) values( '3 PROC_WPSWAGESPROCESS_UPDATE',lv_Sql,lv_Sql,lv_parvalues,lv_remarks);
    ...
    proc_wps_update_NA_comp(P_FNSTDT, P_FNENDT, P_PHASE_TABLENAME,P_TABLENAME,'YES',P_PROCESSTYPE);
