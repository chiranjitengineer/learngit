CREATE OR REPLACE PROCEDURE FORTWILLIAM_WPS.PROC_UNREALIZED_BLNC (
    P_COMPCODE      VARCHAR2,
    P_DIVCODE       VARCHAR2,
...
AS 
    ...
    lv_ProcName    VARCHAR2(30) := 'PROC_UNREALIZED_BLNC';
...
BEGIN
     ...
     DELETE FROM GBL_UNREALIZED_BLNC;
        
LV_SQLSTR := 'INSERT INTO GBL_UNREALIZED_BLNC '|| CHR(10)
        ||' ( '|| CHR(10)
        ||'      WORKERSERIAL, MODULE, COMPONENTCODE, COMPONENT_EMI, COMPONENT_BALANCE '|| CHR(10)
        ||' ) '|| CHR(10)
        ||' SELECT WORKERSERIAL,MODULE, COMPONENTCODE, SUM(COMPONENTAMOUNT) COMPONENT_EMI, SUM(COMPONENTAMOUNT) COMPONENT_BALANCE '|| CHR(10)
        ...
        ||' AND FORTNIGHTENDDATE <= TO_DATE('''||P_ENDDATE||''',''DD/MM/YYYY'') '|| CHR(10)
        ||' GROUP BY WORKERSERIAL,COMPONENTCODE, MODULE '|| CHR(10);
