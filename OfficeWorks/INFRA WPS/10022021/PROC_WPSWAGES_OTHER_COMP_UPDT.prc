CREATE OR REPLACE PROCEDURE INFRA_WPS.PROC_WPSWAGES_OTHER_COMP_UPDT (P_COMPCODE Varchar2, 
                                                  P_DIVCODE Varchar2, 
                                                  P_YEARCODE Varchar2,
...
begin
   ...
   lv_Sql := ' UPDATE '||P_TABLENAME||' SET ESI_COMP_CONT = CEIL(ROUND(ESI_GROSS * '||lv_ESI_C_Percentage||'/100,2)) '||CHR(10)
    ...
    ---CUMMULATIVE FIGURE UPDATE 
    PROC_WPSOTHER_CUMMULATIVE_UPDT(P_COMPCODE, P_DIVCODE,P_YEARCODE, P_FNSTDT, P_FNENDT,101,P_TABLENAME,P_TABLENAME,'');
