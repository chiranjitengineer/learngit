CREATE OR REPLACE PROCEDURE DALHOUSIE_WEB."PROC_SALESBILLPRINTGST_CAN" 
(
...
begin
    ...
    lv_sqlstr :=   'insert into gtt_salesbillprintgst'|| chr(10)
                ...
                || '       S.BILLSERIALNO, S.QUALITYCODE, H.HSNCODE,''[''|| S.QUALITYCODE ||'']  ''|| S.QUALITYMANUALDESC||CHR(10)||S.MARKS QUALITYMANUALDESC, S.PACKINGCODE, S.PACKINGNAME, S.NOOFPACKINGUNIT, S.UORCODE,  '|| chr(10)
                ...
                || '       replace(C.COMPANYNAME,''Unit : '', chr(10)||''Unit : '') COMPANYNAME,''Registered office : ''|| C.COMPANYADDRESS, C.COMPANYADDRESS1, C.COMPANYADDRESS2,''PAN : ''||  C.PANNO, C.COMPANYEMAIL, '|| chr(10)
...
--DBMS_OUTPUT.PUT_LINE(lv_sqlstr);
    ...
    IF IRN_CNT > 0 THEN
        lv_sqlstr:='      UPDATE gtt_salesbillprintgst A  '||CHR(10)
                 ||'         SET (EX12, EX13) = (SELECT IRN, SIGNEDQRCODE FROM EINVOICE_MAIN WHERE A.SALEBILLNO=DOCNO AND DOCNO IN ('||p_billno||'))  '||CHR(10); 
...
--    WHERE EX21='DELIVERY CHALLAN';
    ...
    AND x.serialno='1' AND ROWNUM=1);
     ...
     update gtt_salesbillprintgst a set (TOTALWEIGHT,PACKSHEETWEIGHT,GROSSWEIGHT)
        =
        (
        select TOTALWEIGHT,PACKSHEETWEIGHT,GROSSWEIGHT from(
        select SALEBILLNO,TEXT,sum(nvl(TOTALWEIGHT,0))TOTALWEIGHT,sum(nvl(PACKSHEETWEIGHT,0))PACKSHEETWEIGHT,sum(nvl(GROSSWEIGHT,0))GROSSWEIGHT 
        from gtt_salesbillprintgst 
        group by SALEBILLNO,TEXT
        )b
