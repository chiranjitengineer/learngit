...
BEGIN
    ...
    IF P_ISARREAR = 'Y' THEN
        ...
        LV_RPT_CAPTION:= 'ESI ARREAR STATEMENT FOR THE MONTH OF '||LV_MONTH||', '|| LV_YEAR;
    ELSE
...
LV_SQLSTR := LV_SQLSTR || '    FROM '||LV_TRANS_TABLE||'' || CHR(10);
...
LV_SQLSTR := LV_SQLSTR || 'AND nvl(A.ESIAPPLICABLE,''N'')=''Y''' || CHR(10);
