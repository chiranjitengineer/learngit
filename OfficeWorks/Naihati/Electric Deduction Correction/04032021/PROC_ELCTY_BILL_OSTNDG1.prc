...
SELECT SUBSTR(NVL(A.QUARTERNO,C.QUARTER),1,2) QRTLINENO, SUBSTR(NVL(A.QUARTERNO,C.QUARTER),4,4)QUARTERNO, B.TOKENNO, WORKERNAME, 
            ...
            A.BILLAMOUNT,           
            nvl(fn_PreviousAmount(b.companycode,b.divisioncode,b.workerserial,'''||lv_fortnightstartdate||''','''||lv_fortnightenddate||'''),0)+NVL(A.BILLAMOUNT,0)-NVL(D.DEDUCTEDAMT,0) TOTALAMT,
            ...
            -- end change 
            '''||lv_companyname||''' COMPANYNAME, NVL(D.DEDUCTEDAMT,0) ELECTRICITY
...
----                    AND FORTNIGHTENDDATE <= TO_DATE('''|| p_todate || ''',''DD/MM/YYYY'')
----                    AND A.WORKERSERIAL=WORKERSERIAL
----                ) 
            ) A,
            (
                SELECT WORKERSERIAL, DEDUCTEDAMT FROM ELECTRICDEDUCTIONBREAKUP
                WHERE DEDUCTIONDATE = TO_DATE('''|| p_todate || ''',''DD/MM/YYYY'')
            ) D
            WHERE 1=1
