...
begin
    ...
    if p_processtype = 'SALARY PROCESS' then
       ...
       lv_param_voucher_no := 'AC_VOUCHERNO_PIS_REIMBURSEMENT';
       lv_param_sys_voucher_no := 'AC_SYS_VOUCHERNO_PIS_REIMBURSEMENT';
       lv_salaryorarrear := 'REIMBURSEMENT' ;
    end if;
...
--    lv_sql :=  lv_sql ||' AND  MODULENAME = ''PIS'' '||chr(10);   
    ...
    ELSE
        ...
        lv_sql := lv_sql ||' WHERE COMPANYCODE = '''||p_companycode||''' AND DIVISIONCODE = '''||p_divisioncode||''' '||chr(10);
        lv_sql := lv_sql ||' AND  POSTING_PARAM_NAME = ''PIS SALARY POSTING TO ACCOUNTS'' '||chr(10);
        lv_sql := lv_sql ||' AND  MODULENAME = ''PIS'' '||chr(10); 
    END IF;
