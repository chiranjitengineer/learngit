CREATE OR REPLACE PROCEDURE BIRLANEW.PROC_RPT_AXISBANKPAYROLL_TEXT
(
...
--AND LENGTH(BANKACCNUMBER) = 15
    ...
    LV_SQLSTR := LV_SQLSTR || 'AND LENGTH(BANKACCNUMBER) = 15'|| chr(10) ;    
...
----------------------------------------------------------------------------------------------
            ...
            LV_INSERTSTR := ' AXIX BANK PAYROLL REGISTER FOR THE MONTH OF '||TO_CHAR(TO_DATE(P_YEARMONTH,'YYYYMM'),'MON-YY');
            P_INT1 := P_INT1 + 1;
            PROC_INSERT_TEXT_REPORT(LV_INSERTSTR);
            ...
            LV_INSERTSTR := ' SRL. NO        EMP. CODE     ATM  NUMBER           EMPLOYEE NAME                  NETPAY    |';
            P_INT1 := P_INT1 + 1;
            PROC_INSERT_TEXT_REPORT(LV_INSERTSTR);
...
--            LV_LASTCATEGORY := C1.CATEGORYCODE;
        ...
        LV_INSERTSTR := '  '||FMTALIGN(C1.SLNO,14,'L')||' '||
                        FMTALIGN(C1.TOKENNO,14,'L')||' '||
                        FMTALIGN(C1.ATM_NUMBER,22,'L')||' '||
                        FMTALIGN(C1.EMPLOYEENAME,25,'L')||' '||
                        FMTALIGN(C1.NETPAY,10,'R',2)||' ' ;
