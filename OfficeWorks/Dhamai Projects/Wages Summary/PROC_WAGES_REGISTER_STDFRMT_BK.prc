CREATE OR REPLACE PROCEDURE DHAMAI.PROC_WAGES_REGISTER_STDFRMT_BK
(
...
BEGIN
        ...
        DELETE FROM GTT_WAGES_REGISTER_STDFRMT_BK;
...
LV_SQLSTR :=' INSERT INTO GTT_WAGES_REGISTER_STDFRMT_BK '||CHR(10)
          ||'    SELECT ROWNUM ROWNUMBER, XXX.*    '||CHR(10)
          ||'      FROM (     '||CHR(10)      
          ||'              SELECT A.TOKENNO EMP_CODE, A.PFNO, A.NAME, A.NETPAY, B.SK_DAYS, '||CHR(10) 
          ||'                     B.MT_DAYS, B.AL_DAYS, B.WP_DAYS, B.HOLIDAYS, B.PRESENTDAYS,   '||CHR(10)
          ||'                     D.ATTENDANCEDATE, D.OCCUPATIONCODE, D.DAILYWAGES,  '||CHR(10)
          ...
          ||'                     NVL(E.ADVANCE, 0) ADVANCE, NVL(E.PF_LOAN, 0) PF_LOAN, NVL(E.PF_INT, 0) PF_INT,  '||CHR(10) 
          ...
          ||'                     NVL(E.TOT_DED,0) EX2, NVL(E.OT_AMOUNT,0)  EX3, NVL(E.temp_DED,0) EX4,  E.PF_FC_GROSS EX5  '||CHR(10)
          ||'                FROM (  '||CHR(10)
         ...
         -- ||'                                     CASE WHEN B.OCCUPATIONTYPE IN (''TUBE FILLING'',''PLUCKING'',''OTHERS'') THEN HAZIRA ELSE 0 END PRESENTDAYS  '||CHR(10)
          ...
          ||'                             ATTENDANCEDATE,CASE WHEN B.OCCUPATIONTYPE = ''PLUCKING'' THEN TO_CHAR(TOTALOUTPUT||'' KG'') ELSE B.OCCUPATIONCODE END OCCUPATIONCODE ,A.GROSSWAGES DAILYWAGES  '||CHR(10)
          ||'                        FROM GPSDAILYPAYSHEETDETAILS A  ,GPSOCCUPATIONMAST B,GPSCATEGORYMAST CAT  '||CHR(10)
          ...
          ||'                             GROSSDEDN, COINCF, XX.TOT_EARN, XX.TOT_DED ,PF_FC_GROSS,OT_AMOUNT,temp_DED  '||CHR(10)
...
--            LV_SQLSTR := LV_SQLSTR ||'  ORDER BY A.TOKENNO, D.ATTENDANCEDATE  '||CHR(10)
             LV_SQLSTR := LV_SQLSTR                       ||'  ) XXX   '||CHR(10)
     ...
     DBMS_OUTPUT.PUT_LINE(LV_SQLSTR);
             ...
             UPDATE GTT_WAGES_REGISTER_STDFRMT_BK A
