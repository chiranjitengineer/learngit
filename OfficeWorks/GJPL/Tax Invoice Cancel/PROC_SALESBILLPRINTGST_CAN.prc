CREATE OR REPLACE PROCEDURE GJPL_WEB.PROC_SALESBILLPRINTGST_CAN 
(
...
as 
    ...
    lv_Logo_Path_Blank        varchar(1000);
    lv_Logo_Path_Cancel                varchar(1000);
begin
    select substr(p_logopath,1,length(p_logopath)- 
    instr(reverse(p_logopath),'\')+1) into lv_Logo_Path from dual;
    
    SELECT lv_Logo_Path || LOGONAME INTO lv_Logo_Path_Blank FROM MODULEWISELOGO
    WHERE COMPANYCODE=p_companycode AND DIVISIONCODE=p_divisioncode AND MODULE='SALES' AND LOGO_TYPE='WATERMARK_BLANK' AND ROWNUM=1;
    
    
    SELECT lv_Logo_Path || LOGONAME INTO lv_Logo_Path_Cancel FROM MODULEWISELOGO
    WHERE COMPANYCODE=p_companycode AND DIVISIONCODE=p_divisioncode AND MODULE='SALES' AND LOGO_TYPE='CANCEL' AND ROWNUM=1;
    ...
    lv_sqlstr :=   'insert into gtt_salesbillprintgst'|| chr(10)
                ...
                || '       C.COMPANYNAME,''Mill : ''|| C.COMPANYADDRESS,''Registered office : ''|| C.COMPANYADDRESS1, C.COMPANYADDRESS2,''PAN : ''||  C.PANNO, C.COMPANYEMAIL, '|| chr(10)
                ...
                ||'        '''||lv_Logo_Path_Cancel||''' EX11, NULL EX12, NULL EX13, NULL EX14, NULL EZ15 ,   '||CHR(10)
                ||'        ''FAX : ''||COMPANYFAX EX16, ''COUNTRY : ''|| C.COMPANYCOUNTRY EX17,''STATE : ''||C.COMPANYSTATE EX18,''Registered office : ''|| C.COMPANYADDRESS1/*USED FOR FOOTER PART NOTE*/ EX19,C.COMPANYHOPHONE1 EX20,CASE WHEN S.DOCUMENTTYPE=''SALE MEMO'' AND CHANNELTAG=''EXPORT''  THEN ''DELIVERY CHALLAN'' ELSE ''TAX INVOICE'' END  EX21,NULL /*AmendmentNo. 1*/ EX22,NULL /*AmendmentNo. Date*/ EX23,NULL /*AmendmentNo. 2*/ EX24,NULL /*AmendmentNo. Date*/  EX25,/*S.SHIPPINGINSNO*/NULL EX26,/*TO_CHAR(S.SHIPPINGINSDATE,''DD/MM/YYYY'')*/NULL EX27,CASE WHEN CHANNELTAG=''EXPORT'' THEN S.PROFORMAINVNO ELSE NULL END EX28,CASE WHEN CHANNELTAG=''EXPORT'' THEN TO_CHAR(S.PROFORMAINVDATE,''DD/MM/YYYY'') ELSE NULL END EX29, '||CHR(10)
                ||'        RTRIM(LTRIM(P.PARTYADDRESS)) EX30,S.CONTAINERNO EX31,S.SEALNO EX32,NULL /*E SEAL NO*/ EX33,S.BOOKING EX34,F.LUTNO /*LUT NO*/ EX35,TO_CHAR(F.LUTDATE,''DD/MM/YYYY'')/*LUT DATE*/ EX36,NULL EX37,''Registered office : ''|| C.COMPANYADDRESS1 ||'' ''||C.COMPANYADDRESS2 EX38,NULL EX39,NULL EX40,NULL EX41,NULL EX42,NULL EX43, '||CHR(10)
                ...
                || 'FROM SALESBILLVIEW_CAN S, COMPANYMAST C, DIVISIONMASTER D, PARTYADDRESS P1, PARTYADDRESS P2, PARTYMASTER P, GSTITEMVSHSNMAPPING H, SALESBANKMASTER B,FINANCIALYEAR F, '|| chr(10)
                || '     ( '|| chr(10)
                ...
                || '        FROM SALESCHARGEDETAILS_CAN D, CHARGEMASTER M '|| chr(10)
...
--    WHERE EX21='DELIVERY CHALLAN';
    ...
    (
    SELECT AMMENDMENTNO,TO_CHAR(AMMENDMENTDATE,'DD/MM/YYYY') FROM(
    SELECT DISTINCT AMMENDMENTNO,AMMENDMENTDATE,SALEBILLNO, ROW_NUMBER() OVER(PARTITION BY SALEBILLNO order by SALEBILLNO,SYSTEMAMMENDMENTSERIALNO )SRL
    FROM(
    SELECT DISTINCT A.AMMENDMENTNO,A.AMMENDMENTDATE,B.SALEBILLNO,A.SYSTEMAMMENDMENTSERIALNO  FROM SALESPCSOAMMENDMENTDETAILS A,
    gtt_salesbillprintgst B
    WHERE A.SAUDANO=B.SAUDANO
    AND A.SAUDADATE=B.SAUDADATE
    AND A.COMPANYCODE=p_companycode
    AND A.DIVISIONCODE=p_divisioncode
    AND B.SALEBILLDATE BETWEEN A.VALIDFROM AND A.VALIDTO
    ORDER BY  A.SYSTEMAMMENDMENTSERIALNO
    )
    )B 
    WHERE SRL=1
    AND A.SALEBILLNO=B.SALEBILLNO 
    );
