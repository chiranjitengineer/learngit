INSERT INTO GTT_PISREIMBURSEMENT 
         ...
         A.EMPLOYEENAME,TO_CHAR(A.DATEOFJOIN,'DD/MM/YYYY') DATEOFJOIN , A.UNITCODE,A.CATEGORYCODE, A.GRADECODE, 'FIX_CONV' COMPONENTCODE, 
         B.COMPONENTAMOUNT,'FIX_CONV'   TRANSACTIONTYPE,    'ADD' ADDLESS, 
         ...
         NVL(D3.PREVIOUSYRTAKEN,0)  PREVIOUSYRTAKEN,(B.COMPONENTAMOUNT + (NVL(C.LASTCOMPONENTAMOUNT,0)-NVL(D3.PREVIOUSYRTAKEN,0)) -NVL(D2.PAIDAMOUNT,0)) TOTALDUE,DECODE('A','A', DECODE('FIX_CONV', 'BONUS', B.COMPONENTAMOUNT ,0),NVL(D.PAIDAMOUNT,0))  AMOUNTCLAIMCURRENT,NVL(D4.AMOUNTCLAIMPREVIOUS,0) AMOUNTCLAIMPREVIOUS,
        (B.COMPONENTAMOUNT+(NVL(C.LASTCOMPONENTAMOUNT,0)-NVL(D3.PREVIOUSYRTAKEN,0)))-(NVL(D2.PAIDAMOUNT,0)) BALANCEAMOUNT , NVL(D5.LEAVEBALANCE,0)   LEAVEBALANCE , 0   LEAVEENCASHMENT  
         ...
         (
             SELECT A.WORKERSERIAL, BASIC, DECODE(B.TOKENNO,NULL,A.BASIC,ROUND((A.BASIC/B.YEARDAYS)*B.NOWORKDAYS,2)) COMPONENTAMOUNT FROM 
              (
                  SELECT P.COMPANYCODE, P.DIVISIONCODE,P.UNITCODE, P.GRADECODE, P.CATEGORYCODE, P.DEPARTMENTCODE, P.TOKENNO, P.WORKERSERIAL,  P.YEARMONTH , 0 BASIC
                  ...
                  (
                      ...
                      AND YEARMONTH >= DECODE('FIX_CONV','BONUS','201904','202004')
...
SELECT WORKERSERIAL, TOKENNO, MIN(COMPONENTAMOUNT)  COMPOAMOUNT
FROM PISREIMBURSEMENTDETAILS A
WHERE YEARCODE = '2020-2021'
AND COMPANYCODE='0001'
AND DIVISIONCODE='001' 
AND COMPONENTCODE='FIX_CONV' 
AND YEARMONTH=
(
    SELECT MIN(YEARMONTH) FROM PISREIMBURSEMENTDETAILS   
    WHERE YEARCODE = A.YEARCODE
    AND COMPANYCODE=A.COMPANYCODE
    AND DIVISIONCODE=A.DIVISIONCODE
    AND COMPONENTCODE=A.COMPONENTCODE
)
GROUP BY WORKERSERIAL, TOKENNO 