...
--GROUP BY COMPANYCODE,DIVISIONCODE, LOANCODE,WORKERSERIAL,PFNO 
    ...
    DELETE FROM GTT_LOANMAXDATE WHERE 1=1;
    ...
    LV_SQLSTR :=    'INSERT INTO GBL_PFLOANBLNC '|| CHR(10)
                ...
                ||' CASE WHEN X.DEDUCTIONSTARTDATE > TO_DATE('''||P_ENDDATE||''',''DD/MM/YYYY'') THEN 0 ELSE '||CHR(10)
...
--                ||' CASE WHEN M.DEDUCTIONTYPE = ''DEDUCT CAPITAL FULL THEN INTEREST'' AND SUM(A.AMOUNT) >0 THEN 0 ELSE DECODE(DEDUCTIONTYPE,''DEDUCT CAPITAL AND INTEREST SAMETIME'',NVL(X.INT_EMI,0),NVL(X.TOT_EMI,0)) END END END INT_EMI, '||CHR(10)
    ...
    LV_SQLSTR := LV_SQLSTR    ||' B.MODULE, ''FULL'' CAP_EMI_DEDUCT_TYPE, ''FULL'' INT_EMI_DEDUCT_TYPE  '||chr(10)
                ...
                ||'         FROM PFLOANTRANSACTION A, GTT_LOANMAXDATE B '|| CHR(10)
--                ||'         (  '|| CHR(10)
...
--                ||'         ) B  '|| CHR(10)
                ||'         WHERE A.COMPANYCODE=B.COMPANYCODE AND A.DIVISIONCODE=B.DIVISIONCODE  AND A.LOANCODE = B.LOANCODE  AND A.PFNO = B.PFNO AND A.LOANDATE = B.LOANDATE  AND AMOUNT > 0 '|| CHR(10)
                ||'         UNION ALL     '|| CHR(10)
                ...
                ||'         FROM PFLOANBREAKUP A,  GTT_LOANMAXDATE B'|| CHR(10)
--                ||'         (  '|| CHR(10)
--                ||'             SELECT DISTINCT COMPANYCODE,DIVISIONCODE,PFNO, MAX(LOANDATE) LOANDATE  '|| CHR(10)
--                ||'             FROM PFLOANTRANSACTION  '|| CHR(10)
--                ||'             WHERE COMPANYCODE = '''||P_COMPCODE||''' AND DIVISIONCODE ='''||P_DIVCODE||''' '|| CHR(10)
...
--                ||'             GROUP BY COMPANYCODE,DIVISIONCODE,PFNO  '|| CHR(10)
--                ||'         ) B  '|| CHR(10)
                ||'         WHERE A.COMPANYCODE=B.COMPANYCODE AND A.DIVISIONCODE=B.DIVISIONCODE  AND A.LOANCODE = B.LOANCODE AND A.PFNO = B.PFNO AND A.LOANDATE = B.LOANDATE  '|| CHR(10)
                ...
                ||'                FROM PFLOANTRANSACTION '|| CHR(10)
                ||'                WHERE COMPANYCODE ='''||P_COMPCODE||''' AND DIVISIONCODE = '''||P_DIVCODE||''''|| CHR(10)
                ||'              ) A, GTT_LOANMAXDATE B, PFLOANINTEREST C'|| CHR(10)
--                ||'            (  '|| CHR(10)
--                ||'                 SELECT DISTINCT COMPANYCODE,DIVISIONCODE,PFNO, MAX(LOANDATE) LOANDATE '|| CHR(10) 
--                ||'                FROM PFLOANTRANSACTION  '|| CHR(10)
--                ||'                WHERE COMPANYCODE = '''||P_COMPCODE||''' AND DIVISIONCODE = '''||P_DIVCODE||'''  '|| CHR(10)
...
--                ||'            ) B, PFLOANINTEREST C  '|| CHR(10)
                ...
                ||'         AND A.PFNO = C.PFNO  AND A.LOANCODE = B.LOANCODE AND A.LOANDATE = C.LOANDATE AND A.LOANCODE = C.LOANCODE  '|| CHR(10)
...
--                ||'             GROUP BY COMPANYCODE,DIVISIONCODE,PFNO  '|| CHR(10)
--                ||'         ) B  '|| CHR(10)
                ||'         WHERE A.COMPANYCODE=B.COMPANYCODE AND A.LOANCODE = B.LOANCODE  AND A.DIVISIONCODE=B.DIVISIONCODE AND A.PFNO = B.PFNO AND A.LOANDATE = B.LOANDATE  '|| CHR(10)
                ||'         AND A.EFFECTFORTNIGHT <= TO_DATE('''||P_ENDDATE||''',''DD/MM/YYYY'') '|| CHR(10) 
...
--                ||'         ) B  '|| CHR(10)
                ...
                ||'     (  '||CHR(10)
                ||'        SELECT M.COMPANYCODE,M.DIVISIONCODE,M.PFNO, M.LOANDATE, NVL(M.DEDUCTIONSTARTDATE,M.LOANDATE) DEDUCTIONSTARTDATE ,  '||CHR(10) 
...
--                ||'   AND A.PFNO = Z.PFNO AND A.LOANDATE = Z.LOANDATE '||CHR(10)
                ...
                END IF;
    LV_SQLSTR := LV_SQLSTR ||'  GROUP BY B.WORKERSERIAL, B.TOKENNO, A.PFNO, A.LOANCODE, A.LOANDATE, X.DEDUCTIONSTARTDATE, '||CHR(10)
