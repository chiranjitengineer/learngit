 ...
 INSERT INTO GBL_ELECBLNC 
  ...
  NVL(CONTRIBUTIONAMOUNT,0)+NVL(PREVIOUSDUEAMOUNT,0) - NVL(C.ELEC_BAL_AMT,0) ELEC_BAL_AMT, EMIAMOUNT 
  ...
  ( 
        ...
        AND FORTNIGHTSTARTDATE <= TO_DATE('15/01/2021','DD/MM/YYYY')  
 ...
 insert into GTT_ELECTRIC_OUTTEMP 
    ...
    FORTNIGHTSTARTDATE, FORTNIGHTENDDATE, CONTRIBUTIONAMOUNT, EMIAMOUNT,READINGFROMDATE, READINGTODATE)              
            ...
            FROM ELECTRICMETERREADING A,WPSWAGESDETAILS_MV B
              ...
              AND nvl(B.FORTNIGHTENDDATE,A.FORTNIGHTENDDATE)=TO_DATE('15/01/2021','DD/MM/YYYY')  
...
SELECT SUBSTR(NVL(A.QUARTERNO,C.QUARTER),1,2) QRTLINENO, SUBSTR(NVL(A.QUARTERNO,C.QUARTER),4,4)QUARTERNO, B.TOKENNO, WORKERNAME, 
            nvl(fn_PreviousAmount(b.companycode,b.divisioncode,b.workerserial,'01/01/2021','15/01/2021'),0) BALANCE_OS ,
            to_char(READINGFROMDATE,'MON')  PERIODFROM,
             to_char(READINGTODATE,'MON')  PERIODTO, 
            A.BILLAMOUNT,           
            nvl(fn_PreviousAmount(b.companycode,b.divisioncode,b.workerserial,'01/01/2021','15/01/2021'),0)+NVL(A.BILLAMOUNT,0)-NVL(A.ELECTRICITY,0) TOTALAMT,
            ...
            CASE WHEN DATEOFTERMINATION<=TO_DATE('15/01/2021','DD/MM/YYYY') THEN to_char(DATEOFTERMINATION,'dd/mm/yyyy') ELSE NULL END SUPERANNUATIONDATE,                       
...
SELECT A.WORKERSERIAL, A.TOKENNO, A.QUARTERNO, READINGDATE, BILLAMOUNT, PREVIOUSDUEAMOUNT, 
  NVL(CONTRIBUTIONAMOUNT,0)+NVL(PREVIOUSDUEAMOUNT,0) TOTALCONTRIBUTION,  
  NVL(C.ELEC_BAL_AMT,0) ELEC_BAL_AMT, EMIAMOUNT 
  FROM ELECTRICMETERREADING A, 
  ( 
      SELECT WORKERSERIAL, MAX(FORTNIGHTSTARTDATE) FORTNIGHTSTARTDATE  
      FROM ELECTRICMETERREADING  
...
--              AND A.FORTNIGHTENDDATE=TO_DATE('15/01/2021','DD/MM/YYYY')  
--              AND A.COMPANYCODE=B.COMPANYCODE(+)
--              AND A.DIVISIONCODE=B.DIVISIONCODE(+)
--              AND A.WORKERSERIAL=B.WORKERSERIAL(+)
--              AND A.FORTNIGHTSTARTDATE=B.FORTNIGHTSTARTDATE(+)
...
--              AND C.WORKERSERIAL=A.WORKERSERIAL(+)   
 ...
 SELECT  C.ELEC_BAL_AMT FROM ELECTRICMETERREADING A, 
  ( 
      SELECT WORKERSERIAL, MAX(FORTNIGHTSTARTDATE) FORTNIGHTSTARTDATE  
      FROM ELECTRICMETERREADING  
      WHERE COMPANYCODE = 'NJ0001' 
        AND FORTNIGHTSTARTDATE <= TO_DATE('16/12/2020','DD/MM/YYYY') 
        AND WORKERSERIAL='005402'
      GROUP BY WORKERSERIAL  
  ) B,  
  (  
      SELECT WORKERSERIAL, SUM(ELEC_BAL_AMT) ELEC_BAL_AMT  
  ...
  WHERE A.COMPANYCODE ='NJ0001'
   AND A.DIVISIONCODE = '0002'
  AND A.WORKERSERIAL='005402'
    AND A.WORKERSERIAL=B.WORKERSERIAL   
    AND A.FORTNIGHTSTARTDATE =B.FORTNIGHTSTARTDATE   
    AND A.WORKERSERIAL = C.WORKERSERIAL(+)    
