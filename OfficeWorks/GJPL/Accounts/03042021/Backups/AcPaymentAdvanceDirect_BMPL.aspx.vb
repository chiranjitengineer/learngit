...
Public Class AcPaymentAdvanceDirect_BMPL
    ...
    Public Sub GetTDSData(gridData As String, strNotNullableColumnName As String)
        ...
        Dim DG As New AcPaymentAdvanceDirect_BMPL()
    ...
    Public Shared Function calcTDSAmount(strcompcode As String, strdivcode As String, stryearcode As String,
                    straccode As String, strtdscode As String, strTDSOnAmt As Double, strORDERDATE As String) As String
        Dim strSQL As String
        Dim rcount As Integer = 0
        ...
        strSQL = " prc_DeductTDS('" & strcompcode & "', '" & strdivcode & "','" & stryearcode & "','" & straccode & "','" & strtdscode & "'," & strTDSOnAmt & ",'" & strORDERDATE & "') "
    ...
    <WebMethod> _
    Public Shared Function getTDSGridBySysVoucherNo(strSysVoucherNo As String, strOperationMode As String, strAcCode As String, strSysVoucherDt As String, strGroupType As String, strORDERDATE As String) As String
        Dim dd As New AcPaymentAdvanceDirect_BMPL()
        Dim str As String = dd.GetDataTableJson(dd.TDSGridBySysVoucherNo(strSysVoucherNo, strOperationMode, strAcCode, strSysVoucherDt, strGroupType, strORDERDATE))
        Return str
    End Function
    Private Function TDSGridBySysVoucherNo(strSysVoucherNo As String, strOperationMode As String, strAcCode As String, strSysVoucherDt As String, strGroupType As String, strORDERDATE As String) As DataTable
        Dim dtGridData As New DataTable
        Dim strSQL As String = ""
        Dim rcount As Integer = 0
        ...
        strSQL = strSQL & vbCrLf & "    AND A.COMPANYCODE ='" & HttpContext.Current.Session("COMPANYCODE") & "' "
        strSQL = strSQL & vbCrLf & "    AND A.SYSTEMVOUCHERNO='" & strSysVoucherNo & "' "
        strSQL = strSQL & vbCrLf & "    AND C.WITHEFFECTFROM = (SELECT MAX(D.WITHEFFECTFROM) "
        strSQL = strSQL & vbCrLf & "                            FROM ACTDSMASTER D "
        strSQL = strSQL & vbCrLf & "                            WHERE D.COMPANYCODE=C.COMPANYCODE "
        strSQL = strSQL & vbCrLf & "                            AND D.TDSCODE = C.TDSCODE "
        strSQL = strSQL & vbCrLf & "                            AND D.WITHEFFECTFROM <= A.BILLDATE "
        strSQL = strSQL & vbCrLf & "                            ) "
        ...
        'strSQL = strSQL & vbCrLf & "                             AND B.WITHEFFECTFROM <= TO_DATE('" & strSysVoucherDt & "','DD/MM/YYYY') "
        'strSQL = strSQL & vbCrLf & "                          )"
        strSQL = strSQL & vbCrLf & "    AND B.WITHEFFECTFROM = (SELECT MAX(C.WITHEFFECTFROM)  "
        strSQL = strSQL & vbCrLf & "                            FROM ACTDSMASTER C  "
        strSQL = strSQL & vbCrLf & "                            WHERE C.COMPANYCODE = '" & HttpContext.Current.Session("COMPANYCODE") & "'  "
        strSQL = strSQL & vbCrLf & "                            AND C.TDSCODE = B.TDSCODE  "
        strSQL = strSQL & vbCrLf & "                            AND C.WITHEFFECTFROM <= TO_DATE('" & strSysVoucherDt & "','DD/MM/YYYY')   "
        strSQL = strSQL & vbCrLf & "                           )  "
